...
@section('header_title', 'Gestion des médecins')
@section('header_subtitle', 'Liste et suivi des médecins')
...
<div class="space-y-8 mt-8">
    <div class="flex items-center justify-between p-6 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 mt-8">
        <div class="">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Médecins</h1>
                <p class="text-sm text-gray-500">Liste et gestion des médecins</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <div class="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-sm font-medium border border-blue-200">
                <i class="fas fa-users mr-2"></i> Total : {{ $medecins->total() }}
            </div>
            <a href="{{ route('admin.medecins.create') }}" class="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:shadow-lg hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 font-medium flex items-center gap-2 group">
        ...
        <div class="group relative bg-white rounded-2xl p-6 hover:shadow-2xl transition-all duration-500 cursor-pointer">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <p class="text-sm text-gray-500 font-medium">Total médecins</p>
                    <div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg">
                        <i class="fas fa-user-md text-white text-lg"></i>
                    </div>
                </div>
                <p class="text-3xl font-bold text-gray-800">{{ $medecins->total() }}</p>
            </div>
        </div>
        <div class="group relative bg-white rounded-2xl p-6 hover:shadow-2xl transition-all duration-500 cursor-pointer">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <p class="text-sm text-gray-500 font-medium">Spécialités</p>
                ...
                <p class="text-3xl font-bold text-gray-800">{{ $medecins->pluck('specialite')->unique()->filter()->count() }}</p>
            </div>
        </div>
        <div class="group relative bg-white rounded-2xl p-6 hover:shadow-2xl transition-all duration-500 cursor-pointer">
            ...
            <div class="relative z-10">
                ...
                <p class="text-3xl font-bold text-gray-800">{{ $medecins->where('is_active', true)->count() }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="group hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-indigo-50/50 transition-all duration-300">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-full flex items-center justify-center text-white font-bold shadow-md">
                                    {{ strtoupper(substr($medecin->user->name, 0, 1)) }}
                                </div>
                                <p class="font-semibold text-gray-800">{{ $medecin->user->name }}</p>
                            </div>
                        </td>
                        <td class="px-6 py-4"><span class="font-mono text-sm">{{ $medecin->user->email }}</span></td>
                        <td class="px-6 py-4">
                            @if($medecin->specialite)
                                <span class="px-3 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full">
                                    <i class="fas fa-stethoscope mr-1"></i> {{ $medecin->specialite }}
                                </span>
                            @else
                                <span class="px-3 py-1 text-xs font-medium bg-gray-100 text-gray-500 rounded-full">Non spécifiée</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            @if($medecin->is_active)
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.medecins.edit', $medecin) }}" class="w-8 h-8 flex items-center justify-center text-blue-600 hover:text-white bg-blue-50 hover:bg-blue-600 rounded-lg transition">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.medecins.destroy', $medecin) }}" method="POST" class="delete-form inline">
                                    @csrf @method('DELETE')
                                    <button type="button" class="delete-btn w-8 h-8 flex items-center justify-center text-red-600 hover:text-white bg-red-50 hover:bg-red-600 rounded-lg transition" data-name="{{ $medecin->user->name }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-16">Aucun médecin trouvé</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
