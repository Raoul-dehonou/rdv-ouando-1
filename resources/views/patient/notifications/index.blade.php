...
@section('header_subtitle', 'Recevez vos alertes et rappels')
...
<div class="max-w-4xl mx-auto space-y-8 mt-8">
    <div class="flex items-center justify-between p-6 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
        ...
        <form action="{{ route('patient.notifications.read-all') }}" method="POST">
            @csrf
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="p-6 {{ $notif->read_at ? 'bg-white hover:bg-gray-50' : 'bg-blue-50 hover:bg-blue-100' }} transition-all duration-300">
                <div class="flex justify-between items-start">
                    ...
                    @if(!$notif->read_at)
                    <form action="{{ route('patient.notifications.read', $notif) }}" method="POST" class="ml-4">
                        @csrf
                        <button type="submit" class="text-blue-600 hover:text-blue-800 text-sm font-medium transition">
                            Marquer comme lu
                        </button>
                    </form>
                    @endif
